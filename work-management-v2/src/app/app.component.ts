...
export class AppComponent implements OnInit, OnDestroy {
  ...
  setBreakpointsForHeader() {
    const break800$ = this.bpService
      .observeBreakpoint()
      .subscribe((state: BreakpointState) => {
        state.matches && !this.bpService.over1200
          ? (this.bpService.isHeader = false)
          : (this.bpService.isHeader = true);
      });
  ...
  setUserHours() {
    const hours$ = this.sessionQuery.uid$
      .pipe(filter((el) => el !== ''))
      .pipe(
        switchMap((uid) =>
          uid ? this.hoursQuery.selectFetchOrStore(uid) : of([])
        )
      )
      .subscribe();
  ...
  setUserWorktypes() {
    const worktypes$ = this.sessionQuery.uid$
      .pipe(filter((el) => el !== ''))
      .pipe(switchMap((uid) => this.worktypeQuery.selectFetchOrStore(uid)))
      .subscribe();
  ...
  setUserProfile() {
    ...
    this.photo$ = this.sessionQuery.allState$.pipe(map((el) => el.photoUrl));
  }
